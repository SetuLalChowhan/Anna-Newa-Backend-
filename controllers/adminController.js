  ...
  try {
    ...
    res.json({
      ...
      statistics: {
        ...
        purchased:
          productStats.find((stat) => stat._id === "purchased")?.count || 0,
        expired:
          productStats.find((stat) => stat._id === "expired")?.count || 0,
      },
      pagination: {
        page: pageNum,
        limit: limitNum,
    ...
    res.json({
      ...
      dashboard: {
        ...
        summary: {
          ...
          totalRevenue: parseFloat(financials.totalRevenue.toFixed(2)),
          totalTransactions: parseFloat(
            financials.totalTransactions.toFixed(2)
          ), // Total sales
          totalSellerPayout: parseFloat(
            financials.totalSellerPayout.toFixed(2)
          ), // 98% to sellers
        ...
        products: {
          active:
            productStats.find((stat) => stat._id === "active")?.count || 0,
          sold: productStats.find((stat) => stat._id === "sold")?.count || 0,
          purchased:
            productStats.find((stat) => stat._id === "purchased")?.count || 0,
        },
        ...
        orders: {
          ...
          processing:
            orderStats.find((stat) => stat._id === "Processing")?.count || 0,
          cancelled:
        ...
        financials: {
          ...
          averageOrderValue:
            financials.totalOrders > 0
              ? parseFloat(
                  (
                    financials.totalTransactions / financials.totalOrders
                  ).toFixed(2)
                )
              : 0,
        },
